...
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_BIAS_V_OFFSET) = value;
}

uint16_t control_unit_bias_v_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_BIAS_V_OFFSET);
}

void control_unit_black_level_set(control_unit_core_state_t *state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_BLACK_LEVEL_OFFSET) = value;
}

uint16_t control_unit_black_level_get(control_unit_core_state_t *state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_BLACK_LEVEL_OFFSET);
}

void control_unit_bpc_en_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_BPC_EN_OFFSET) = value;
}

uint16_t control_unit_bpc_en_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_BPC_EN_OFFSET);
}

void control_unit_drift_enable_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_DRIFT_ENABLE_OFFSET) = value;
}

uint16_t control_unit_drift_enable_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_DRIFT_ENABLE_OFFSET);
}

void control_unit_drift_position_set(control_unit_core_state_t *state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_DRIFT_POSITION_OFFSET) = value;
}

uint16_t control_unit_drift_position_get(control_unit_core_state_t *state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_DRIFT_POSITION_OFFSET);
}

void control_unit_int_time_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_INT_TIME_OFFSET) = value;
}

uint16_t control_unit_int_time_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_INT_TIME_OFFSET);
}

void control_unit_offset_update_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_OFFSET_UPDATE_OFFSET) = value;
}

uint16_t control_unit_offset_update_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_OFFSET_UPDATE_OFFSET);
}

void control_unit_shutter_reset_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_SHUTTER_RESET_OFFSET) = value;
}

uint16_t control_unit_shutter_reset_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_SHUTTER_RESET_OFFSET);
}

void control_unit_shutter_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_SHUTTER_OFFSET) = value;
}

uint16_t control_unit_shutter_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_SHUTTER_OFFSET);
}

uint16_t control_unit_temp1_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_TEMP1_OFFSET);
}

void control_unit_temp1_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_TEMP1_OFFSET) = value;
}

uint16_t control_unit_temp2_get(control_unit_core_state_t* state)
{
    return *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_TEMP2_OFFSET);
}

void control_unit_temp2_set(control_unit_core_state_t* state, uint16_t value)
{
    *((uint16_t*)(((uint8_t*)state->priv)) + NIT_CONTROL_UNIT_TEMP2_OFFSET) = value;
}
