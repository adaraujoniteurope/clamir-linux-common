...
int LoadBPCtable(volatile int *shmemProcVar)
{
	...
	else
	{
		...
		if (fscanf(fdBPC, "%s", SerialNumber) != 0)
		{
			...
			memcpy((void *)shmemProcVar + (SERIAL_NUMBER_LOW * 4), SerialNumber, 8);
...
void ImageWriter(int newsockimgfd, volatile int *shmemMeta, volatile int *shmem)
{
	...
	while (nw >= 0)
	{
		...
		if (nw < 0)
			error("ERROR writing image to socket\n");
...
/*
 * Funci�n de gestion de comandos
 */
...
int main()
{
	...
	/*
	 * Crea un hilo de procesamiento unico para la funcion de control en lazo cerrado del laser, completamente independiente de la conexion TCP con el ordenador
	 */
	pid_t pidControl;
	pidControl = fork();
	if (pidControl == 0)
	{
		...
		pthread_mutex_lock(&lock);
		CPU_SET(1, &mask_c);
		sched_setaffinity(0, sizeof(&mask_c), &mask_c);
		if (pthread_create(&log_thrd, NULL, Logger, (void *)shmemMeta) != 0)
		{
	...
	{
		printf("Proceso de control en lazo cerrado creado con PID %d\n", pidControl);
		cpu_set_t mask_tcp;
		CPU_ZERO(&mask_tcp);
		CPU_SET(0, &mask_tcp);
		sched_setaffinity(0, sizeof(&mask_tcp), &mask_tcp);

		// Variables del padre
		pid_t pidCommands, pidImages;
		...
		if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &set_reuseaddress, sizeof(set_reuseaddress)) < 0)
			error("Unable to use REUSEADDR 1");
		...
		/*
		 * Socket de Puerto de comunicacion de imagenes
		 */

		sockimgfd = socket(AF_INET, SOCK_STREAM, 0);
		if (sockimgfd < 0)
			error("ERROR opening socket");
		bzero((char *)&serv_addr_img, sizeof(serv_addr_img));
		portimg = 4096;
		serv_addr_img.sin_family = AF_INET;
		serv_addr_img.sin_addr.s_addr = INADDR_ANY;
		serv_addr_img.sin_port = htons(portimg);
		if (setsockopt(sockimgfd, SOL_SOCKET, SO_REUSEADDR, &set_reuseaddress, sizeof(set_reuseaddress)) < 0)
			error("Unable to use REUSEADDR 2");
		if (bind(sockimgfd, (struct sockaddr *)&serv_addr_img, sizeof(serv_addr_img)) < 0)
			error("ERROR on binding 2");
		listen(sockimgfd, 5);
		cliimglen = sizeof(cli_addr_img);

		/*
		 * Bucle de reentrada en caso de que el cliente corte la conexion
		 */
		while (1)
		{
			newsockfd = accept(sockfd, (struct sockaddr *)&cli_addr, &clilen);
			if (newsockfd < 0)
				error("ERROR on accept 4097");
			else
			{
				pidCommands = fork();
				if (pidCommands == 0)
				{
					close(sockfd);
					CommandHost(newsockfd, shmem, shmemProcVar, ptrAIMENCom, shmemReset);
					close(newsockfd);
					exit(EXIT_SUCCESS);
				}
				else
				{
					newsockimgfd = accept(sockimgfd, (struct sockaddr *)&cli_addr_img, &cliimglen);
					if (newsockimgfd < 0)
						error("ERROR on accept 4096");
					else
					{
						pidImages = fork();
						if (pidImages == 0)
						{
							close(sockimgfd);
							ImageWriter(newsockimgfd, shmemMeta, shmem);
							close(newsockimgfd);
							exit(EXIT_SUCCESS);
						}
						else
						{ // Gestion del padre
							printf("Proceso de escucha con PID %d. Proceso de escritura con PID %d.\n", pidCommands, pidImages);
							close(newsockfd);
							close(newsockimgfd);
							signal(SIGCHLD, SIG_IGN);
							// wait(NULL); //en teor�a esto espera a que termine uno de los hijos
						}
					}
				}
			}
		...
		close(fdProccommand);
		munmap((int *)ptrAIMENCom, 5242888);
		ptrAIMENCom = NULL;
		close(fd);
		munmap((int *)shmem, BRAM_NIT_SIZE);
		shmem = NULL;
		munmap((int *)shmemMeta, 256);
		shmemMeta = NULL;
		munmap((int *)shmemProcVar, 256);
		shmemProcVar = NULL;
		close(fdProccommand);
		close(fdR);
		munmap((int *)shmemReset, 131072);
		shmemReset = NULL;
