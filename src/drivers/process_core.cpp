...
#define END_OF_PROCESS                                                         \
  12 // imagenes sin se�al para considerar un proceso finalizado
...
nit_process_core_private_state *
nit_process_core_process_data_ptr_get(nit_process_core_state_t *state);

int nit_process_core_open(
    nit_process_core_state_t *state, nit_arm_core_state_t *nit_arm_core_state,
    nit_control_unit_core_state_t *nit_control_unit_core_state,
    nit_mb_core_state_t *nit_mb_core_state,
    nit_framebuffer_core_state_t *nit_framebuffer_core_state) {
  if (state == NULL) {
    return -1;
  }
  ...
  process->proc_var_shm = (volatile int *)memory_map_open(
      NULL, 512, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);

  process->virtual_metadata_shm = (volatile int *)memory_map_open(
      NULL, 256, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  ...
  process->real_metadata_shm =
      (volatile int *)nit_framebuffer_core_metadata(nit_framebuffer_core_state);
...
volatile int *
nit_process_core_get_virtual_metadata_shm_ptr(nit_process_core_state_t *state) {
...
int nit_process_core_config_save_to_file(nit_process_core_state_t *state,
                                         const char *path) {
  ...
  if (nit_process_core_power_limit_max_get(
          state, &state->config.power_limit_max) < 0) {
    return -7;
  }
  if (nit_process_core_power_limit_min_get(
          state, &state->config.power_limit_min) < 0) {
    return -8;
  }
  if (nit_process_core_set_ref_width_get(state, &state->config.set_ref_width) <
      0) {
    return -9;
  }
  ...
  if (nit_process_core_pixel_mm_ratio_get(state,
                                          &state->config.pixel_mm_ratio) < 0) {
    return -11;
  }
  if (nit_process_core_pid_error_get(state, &state->config.pid_error) < 0) {
    return -12;
  }
  if (nit_process_core_end_of_process_get(state,
                                          &state->config.end_of_process) < 0) {
    return -13;
  }
  if (nit_process_core_limit_integral_get(state,
                                          &state->config.limit_integral) < 0) {
    return -14;
  }
  ...
  if (nit_process_core_background_remove_get(
          state, &state->config.background_remove) < 0) {
    return -17;
  }
  if (nit_process_core_enable_alarm_get(state, &state->config.enable_alarm) <
      0) {
    return -18;
  }
  ...
  if (nit_process_core_serial_number_low_get(
          state, &state->config.serial_number_low) < 0) {
    return -22;
  }
  if (nit_process_core_serial_number_high_get(
          state, &state->config.serial_number_high) < 0) {
    return -23;
  }
  if (nit_process_core_automeasure_get(state, &state->config.automeasure) < 0) {
    return -24;
  }
  if (nit_process_core_autoshutter_config_get(
          state, &state->config.autoshutter_config) < 0) {
    return -25;
  }
  if (nit_process_core_autoshutter_temp_get(
          state, &state->config.autoshutter_temp) < 0) {
    return -26;
  }
  if (nit_process_core_autoshutter_timer_get(
          state, &state->config.autoshutter_timer) < 0) {
    return -27;
  }
  if (nit_process_core_track_ref_start_get(
          state, &state->config.track_ref_start) < 0) {
    return -28;
  }
  if (nit_process_core_laser_external_control_get(
          state, &state->config.laser_external_control) < 0) {
    return -29;
  }
  if (nit_process_core_delay_laser_on_get(state,
                                          &state->config.delay_laser_on) < 0) {
    return -30;
  }
  if (nit_process_core_preheating_ena_get(state,
                                          &state->config.preheating_ena) < 0) {
    return -31;
  }
  if (nit_process_core_preheating_time_get(
          state, &state->config.preheating_time) < 0) {
    return -32;
  }
  if (nit_process_core_preheating_power_get(
          state, &state->config.preheating_power) < 0) {
    return -33;
  }
...
int nit_process_core_config_load_from_file(nit_process_core_state_t *state,
                                           const char *path) {
  ...
  if (nit_process_core_power_limit_max_set(state,
                                           state->config.power_limit_max) < 0) {
    return -7;
  }
  if (nit_process_core_power_limit_min_set(state,
                                           state->config.power_limit_min) < 0) {
    return -8;
  }
  if (nit_process_core_set_ref_width_set(state, state->config.set_ref_width) <
      0) {
    return -9;
  }
  ...
  if (nit_process_core_pixel_mm_ratio_set(state, state->config.pixel_mm_ratio) <
      0) {
    return -11;
  }
  ...
  if (nit_process_core_end_of_process_set(state, state->config.end_of_process) <
      0) {
    return -13;
  }
  if (nit_process_core_limit_integral_set(state, state->config.limit_integral) <
      0) {
    return -14;
  }
  ...
  if (nit_process_core_background_remove_set(
          state, state->config.background_remove) < 0) {
    return -17;
  }
  if (nit_process_core_enable_alarm_set(state, state->config.enable_alarm) <
      0) {
    return -18;
  }
  ...
  if (nit_process_core_serial_number_low_set(
          state, state->config.serial_number_low) < 0) {
    return -22;
  }
  if (nit_process_core_serial_number_high_set(
          state, state->config.serial_number_high) < 0) {
    return -23;
  }
  ...
  if (nit_process_core_autoshutter_config_set(
          state, state->config.autoshutter_config) < 0) {
    return -25;
  }
  if (nit_process_core_autoshutter_temp_set(
          state, state->config.autoshutter_temp) < 0) {
    return -26;
  }
  if (nit_process_core_autoshutter_timer_set(
          state, state->config.autoshutter_timer) < 0) {
    return -27;
  }
  if (nit_process_core_track_ref_start_set(state,
                                           state->config.track_ref_start) < 0) {
    return -28;
  }
  if (nit_process_core_laser_external_control_set(
          state, state->config.laser_external_control) < 0) {
    return -29;
  }
  if (nit_process_core_delay_laser_on_set(state, state->config.delay_laser_on) <
      0) {
    return -30;
  }
  if (nit_process_core_preheating_ena_set(state, state->config.preheating_ena) <
      0) {
    return -31;
  }
  if (nit_process_core_preheating_time_set(state,
                                           state->config.preheating_time) < 0) {
    return -32;
  }
  if (nit_process_core_preheating_power_set(
          state, state->config.preheating_power) < 0) {
    return -33;
  }
...
int nit_process_core_run(nit_process_core_state_t *state,
  ...
  auto led_controller =
      nit::embedded::drivers::led_core::create(0x40001000, 4096, "/dev/mem");
  ...
  priv->temperature_last_autoshutter =
      ((float)priv->virtual_metadata_shm[13]) / 10;
  ...
  auto *fifo_ctrl_ptr = (int *)mmap(NULL, _SC_PAGE_SIZE, PROT_READ | PROT_WRITE,
                                    MAP_SHARED, fifo_ctrl_fd, 0x40003000);

  auto frame_metadata = [&]() -> int * {
    return (int *)(((uint8_t *)priv->real_metadata_shm) +
                   (*fifo_ctrl_ptr * 0x2100));
  };
  ...
  while (!shutdown) {
    ...
    priv->pixel_mm_ratio = ((double)priv->proc_var_shm[PIXEL_MM_RATIO] /
                            1000); // se reciben en micras
    ...
    priv->autoshutter_config =
        priv->proc_var_shm[AUTOSHUTTER_CONFIG]; // booleano, comprobar flags
    // decode_config
    priv->auto_auto_shutter_enable = priv->autoshutter_config & 0x01;
    priv->auto_auto_shutter_enable_inprocess =
        (priv->autoshutter_config >> 1) & 0x01;
    priv->auto_auto_shutter_time_xtemp = (priv->autoshutter_config >> 3) & 0x01;
    // decodee config end
    priv->autoshutter_temp_target =
        ((float)priv->proc_var_shm[AUTOSHUTTER_TEMP]) /
        10; // de int16 recibido a double o float, dividir entre 10
    priv->autoshutter_time_target =
        priv->proc_var_shm[AUTOSHUTTER_TIMER] *
        10000; //	de int16 recibido a int32, multiplicar por 10
    ...
    // read(fd_int, (int *)&pending, sizeof(int)); // Se bloquea hasta que
    // sucede una interrupcion de uio0
    ...
    // write(fd_int, (void *)&enable, sizeof(int)); // Habilita la interrupcion
    // de uio0
    ...
    if (priv->proc_var_shm[AUTO_SHUTTER]) {
      ...
      if (!priv->measurement_NAP) {
        ...
        priv->cont_calibration = WAIT_START_CALIBRATION + 1 +
                                 WAIT_STOP_CALIBRATION + WAIT_SECOND_APERTURE;
    ...
    if (!priv->measurement_NAP) {
      ...
      priv->w_circ_buffer[priv->z] =
          priv->pixel_mm_ratio *
          nit_process_core_calculate_width(priv->metadatos);
      priv->W_circ_buff_aux = 0;
      ...
      priv->W = priv->W;
      if ((priv->cont_calibration <= 0) &&
          (priv->control_unit_shm[NIT_SHUTTER] != 0)) {
        ...
        if (priv->cont_calibration ==
            (1 + WAIT_STOP_CALIBRATION + WAIT_SECOND_APERTURE)) {
    ...
    case IDLE:
      ...
      if (priv->laser_status) {
        ...
        priv->track_length =
            (((priv->track_length << 32) |
              ((unsigned int)priv->mb_core_shm[TIME_TRACK_LOW])) *
             1000) /
            CLK_100MHZ;
      } else {
        priv->potencia_t0 = priv->proc_var_shm[POWER_MAN];
    ...
    case MIDIENDO:
      ...
      if (priv->mode == 0) {
        ...
        if ((priv->laser_status > 0) &&
            (priv->laser_status != priv->last_laser_status)) {
          if (priv->laser_external) {
            priv->track++;
      ...
      // if ((laser_status) & (metadatos[7] >= proc_var_shm[TRACK_REF_START]))
      // //sustituido por cuenta de tracks en el ARM
      if ((priv->laser_status) &
          (priv->track >= priv->proc_var_shm[TRACK_REF_START])) {
    ...
    case CONTROL:
      ...
      if (priv->delay_laser_on > 0) {
        ...
        if ((!priv->laser_status) || (priv->control_NAP)) {
          ...
          //				derivative = (error_t0 - error_t1)/dt;
          ////comentada por posibilidad de valor muy grande
          priv->derivative = (priv->error_t0 - priv->error_t1);
          priv->potencia_t0 =
              priv->proc_var_shm[POWER_MAN] + (priv->kp * priv->error_t0) +
              (priv->ki * priv->integral) + (priv->kd * priv->derivative);
    ...
    priv->laser_region_width =
        (((int)(100 * priv->width_ref) << 16)) | ((int)(priv->W * 100));
    ...
    priv->metadatos[11] = (priv->estadoAutomata << 24) |
                          (priv->laser_status << 16) | priv->metadatos[11];

    priv->duty = (priv->potencia_t0 - priv->potenciaMin) *
                 (16383 / (priv->potenciaMax - priv->potenciaMin));
...

    {
      ...
      auto voltage = unsafe_get<nit_control_unit_core_state_t, uint16_t>(
          &nit_control_unit_core_driver, nit_control_unit_core_temp1_offset);
...

    {
      /** because of speed we ignore driver access assertions */
      auto voltage = unsafe_get<nit_control_unit_core_state_t, uint16_t>(
          &nit_control_unit_core_driver, nit_control_unit_core_temp2_offset);
    ...
    if (priv->auto_auto_shutter_time_xtemp) {
      ...
      if (priv->autoshutter_temp_target <
          (fabs(priv->temperature_last_autoshutter -
                priv->current_temperature))) {
        priv->pending_autoshutter = 1;
        priv->cnt_last_autoshutter = 0;
        priv->temperature_last_autoshutter = priv->current_temperature;
      }
    ...
    case IDLE:
      ...
      break;
    case MIDIENDO:
      if (priv->auto_auto_shutter_enable &&
          priv->auto_auto_shutter_enable_inprocess &&
          priv->pending_autoshutter) {
        priv->proc_var_shm[AUTO_SHUTTER] = 1;
        priv->pending_autoshutter = 0;
      }
    ...
    if (priv->alarm_enable == 1) { // Solo si la alarma est� habilitada
      priv->mb_core_shm[DIGITAL_OUT_0] = priv->alarm;
    }

    sem_getvalue(priv->semaforo, &priv->valor1);
    if (priv->valor1 < 1) { // permite al productor enviar hasta 2 imagenes por
                            // TCP al empezar la conexion
...
template <typename state_type, typename type>
constexpr type process_core_unsafe_set(state_type *state, size_t offset,
                                       type value) {
  auto priv = (nit_process_core_private_state *)state->priv;
  return (type)(*(((uint32_t *)priv->proc_var_shm) + offset) = value);
}

#define DRIVER_FIELD_AS_FUNCTION_DEFINITION(name, parameter, type, size,       \
                                            offset)                            \
  int nit_##name##_##parameter##_set(nit_##name##_state_t *state,              \
                                     type value) {                             \
    syslog(LOG_INFO, "%s: (%04x): %d\n", __func__, offset, value);             \
    int retval = 0;                                                            \
    ...
    return retval;                                                             \
                                                                               ...
                                                                               \
  int nit_##name##_##parameter##_get(nit_##name##_state_t *state,              \
                                     type *value) {                            \
    int retval = 0;                                                            \
                                                                               \
    if ((retval = nit_##name##_assert(state)) != 0) {                          \
...
double nit_process_core_calculate_width(int metadatos[12]) {
  ...
  W = sqrt(
      8 * (u20 + u02 -
           sqrt((4 * u11 * u11) +
                ((u20 - u02) * (u20 - u02))))); // Aqui se ha calculado el ancho
  return W;
}
