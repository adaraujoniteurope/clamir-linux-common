...
int nit_mb_core_open(nit_mb_core_state_t *state) {
  ...
  state->priv = (volatile int *)memory_map_open(
      NULL, NIT_MB_CORE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, state->fd,
...
int nit_mb_core_close(nit_mb_core_state_t *state) {
  ...
  if (state->priv != NULL) {
    memory_map_close((void *)state->priv, NIT_MB_CORE_SIZE);
  }
...
int nit_mb_core_config_save_to_file(nit_mb_core_state_t *state,
                                    const char *path) {
...
int nit_mb_core_config_load_from_file(nit_mb_core_state_t *state,
                                      const char *path) {
