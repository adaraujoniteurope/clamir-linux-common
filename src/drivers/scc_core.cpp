...
int nit_scc_core_open(
    nit_scc_core_state_t *state,
  ...
  state->priv =
      (scc_core_private_state_t *)malloc(sizeof(scc_core_private_state_t));
  ...
  priv->framebuffer_shm = (int16_t *)nit_framebuffer_core_get_memory_map(
      nit_framebuffer_core_state);
  ...
  priv->ctrl_shm = (volatile uint32_t *)memory_map_open(
      NULL, NIT_SCC_CORE_CTRL_BASE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,
      priv->dev_fd, NIT_SCC_CORE_CTRL_BASE_ADDRESS);
   ...
   * startup core
   */
  priv->ctrl_stub_shm = (volatile uint32_t *)memory_map_open(
      NULL, NIT_SCC_CORE_CTRL_STUB_BASE_SIZE, PROT_READ | PROT_WRITE,
      MAP_SHARED, priv->dev_fd, NIT_SCC_CORE_CTRL_STUB_BASE_ADDRESS);
  ...
  priv->offset_shm = (volatile int32_t *)memory_map_open(
      NULL, NIT_SCC_CORE_OFFSET_BASE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,
      priv->dev_fd, NIT_SCC_CORE_OFFSET_BASE_ADDRESS);
  ...
  priv->scale_shm = (volatile int32_t *)memory_map_open(
      NULL, NIT_SCC_CORE_SCALE_BASE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,
      priv->dev_fd, NIT_SCC_CORE_SCALE_BASE_ADDRESS);
  ...
  // size_t frame_size = sizeof(int16_t) * state->config.width *
  // state->config.height;
  ...
  if (state->config.frame_offset_default_file_path.empty()) {
    state->config.frame_offset_default_file_path =
        "scc_core_offset_default.dat";
  }

  size_t scale_size =
      sizeof(int32_t) * state->config.width * state->config.height;

  if (std::filesystem::exists(state->config.frame_scale_default_file_path)) {
    if (std::filesystem::file_size(
            state->config.frame_scale_default_file_path) != scale_size) {
      syslog(LOG_INFO, "removing invalid SCC scale table size at path: %s",
             state->config.frame_offset_default_file_path.c_str());
      std::filesystem::remove(state->config.frame_scale_default_file_path);

      syslog(LOG_INFO, "initializing default scc core scale table at path: %s",
             state->config.frame_offset_default_file_path.c_str());
      if (scc_core_scale_default_initialize(state) < 0) {
        syslog(LOG_INFO, "failed to initialize default scale at path: %s",
               state->config.frame_offset_default_file_path.c_str());
        return -1;
      }
    }
  ...
  if (std::filesystem::exists(state->config.frame_offset_default_file_path)) {
    if (std::filesystem::file_size(
            state->config.frame_offset_default_file_path) != offset_size) {
      syslog(LOG_INFO, "removing invalid SCC offset table size at path: %s",
             state->config.frame_offset_default_file_path.c_str());
      std::filesystem::remove(state->config.frame_offset_default_file_path);

      syslog(LOG_INFO, "initializing default scc core offset table at path: %s",
             state->config.frame_offset_default_file_path.c_str());
      if (scc_core_offset_default_initialize(state) < 0) {
        syslog(LOG_INFO, "failed to initialize default offset at path: %s",
               state->config.frame_offset_default_file_path.c_str());
        return -1;
      }
    }
  ...
  if (scc_core_scale_default_load(state) < 0) {
    syslog(
        LOG_INFO, "failed to load defailt scale from file: %s",
        std::filesystem::absolute(state->config.frame_scale_default_file_path)
            .c_str());
  }
  ...
  if (scc_core_scale_default_apply(state) < 0) {
    syslog(LOG_INFO, "failed to apply defailt scale");
  }
...
int scc_core_scale_default_load(nit_scc_core_state_t *state) {
  scc_core_private_state_t *priv = (scc_core_private_state_t *)state->priv;
  auto fd = ::open(state->config.frame_scale_default_file_path.c_str(),
                   O_CREAT | O_RDWR);

  if (fd < 0) {
    syslog(LOG_INFO, "Failed to open default scale path at %s",
           state->config.frame_scale_default_file_path.c_str());
    return -1;
  }
...
int scc_core_offset_default_initialize(nit_scc_core_state_t *state) {
  auto fd = ::open(state->config.frame_offset_default_file_path.c_str(),
                   O_CREAT | O_RDWR);

  if (fd < 0) {
    syslog(LOG_INFO, "Failed to open default offset path at %s",
           state->config.frame_offset_default_file_path.c_str());
    return -1;
  }
  ...
  for (size_t rows = 0; rows < state->config.height; rows++) {
    for (size_t cols = 0; cols < state->config.width; cols++) {
      ::write(fd, &state->config.frame_pixel_offset_default_value,
              sizeof(int32_t));
    }
  }
...
int scc_core_scale_default_initialize(nit_scc_core_state_t *state) {
  ...
  for (size_t rows = 0; rows < state->config.height; rows++) {
    for (size_t cols = 0; cols < state->config.width; cols++) {
      ::write(fd, &state->config.frame_pixel_scale_default_value,
              sizeof(int32_t));
    }
  }
...
int nit_scc_core_config_save_to_file(nit_scc_core_state_t *state,
                                     const char *path) {
...
int nit_scc_core_config_load_from_file(nit_scc_core_state_t *state,
                                       const char *path) {
...
int scc_core_config_save_to_file(nit_scc_core_state_t *state,
                                 const char *path) {
  return config_file_save_to_file(state, path);
}

int scc_core_config_load_from_file(nit_scc_core_state_t *state,
                                   const char *path) {
  return config_file_load_from_file(state, path);
}

int nit_scc_core_testing_disable_set(nit_scc_core_state_t *state,
                                     uint32_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_testing_disable_offset, value);
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *((uint32_t *)((uint8_t *)priv->ctrl_shm) +
    nit_scc_core_testing_disable_offset) = value;

  return retval;
}

int nit_scc_core_testing_disable_get(nit_scc_core_state_t *state,
                                     uint32_t *value) {
  int retval = 0;
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_shm) +
             nit_scc_core_testing_disable_offset);
...
int nit_scc_core_testing_addr_set(nit_scc_core_state_t *state, uint32_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_testing_addr_offset, value);
...
int nit_scc_core_testing_addr_get(nit_scc_core_state_t *state,
                                  uint32_t *value) {
  int retval = 0;
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_shm) +
             nit_scc_core_testing_addr_offset);
...
int nit_scc_core_testing_wren_set(nit_scc_core_state_t *state, uint32_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_testing_wren_offset, value);
...
int nit_scc_core_testing_wren_get(nit_scc_core_state_t *state,
                                  uint32_t *value) {
  int retval = 0;
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_shm) +
             nit_scc_core_testing_wren_offset);
...
int nit_scc_core_testing_data_set(nit_scc_core_state_t *state, uint32_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_testing_data_offset, value);
...
int nit_scc_core_testing_data_get(nit_scc_core_state_t *state,
                                  uint32_t *value) {
  int retval = 0;
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_shm) +
             nit_scc_core_testing_data_offset);
...
int nit_scc_core_opmode_set(nit_scc_core_state_t *state, uint16_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__, nit_scc_core_opmode_offset,
         value);
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) + nit_scc_core_opmode_offset) =
      value;
...
int nit_scc_core_opmode_get(nit_scc_core_state_t *state, uint16_t *value) {
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) +
             nit_scc_core_opmode_offset);
...
int nit_scc_core_process_bypass_get(nit_scc_core_state_t *state,
                                    uint16_t *value) {
  int retval = 0;
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) +
             nit_scc_core_process_bypass_offset);

  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_process_bypass_offset, *value);
  return retval;
}

int nit_scc_core_process_bypass_set(nit_scc_core_state_t *state,
                                    uint16_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_process_bypass_offset, value);
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) +
    nit_scc_core_process_bypass_offset) = value;

  return retval;
}

int nit_scc_core_calibration_bypass_get(nit_scc_core_state_t *state,
                                        uint16_t *value) {
  int retval = 0;
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) +
             nit_scc_core_calibration_bypass_offset);

  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_calibration_bypass_offset, *value);
  return retval;
}

int nit_scc_core_calibration_bypass_set(nit_scc_core_state_t *state,
                                        uint16_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_calibration_bypass_offset, value);
...
int nit_scc_core_width_get(nit_scc_core_state_t *state, uint16_t *value) {
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) +
             nit_scc_core_width_offset);

  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__, nit_scc_core_width_offset,
         *value);
  return retval;
}
...
int nit_scc_core_width_set(nit_scc_core_state_t *state, uint16_t value) {
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) + nit_scc_core_width_offset) =
      value;
...
int nit_scc_core_height_get(nit_scc_core_state_t *state, uint16_t *value) {
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) +
             nit_scc_core_height_offset);

  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__, nit_scc_core_height_offset,
         *value);
  return retval;
}
...
int nit_scc_core_height_set(nit_scc_core_state_t *state, uint16_t value) {
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) + nit_scc_core_height_offset) =
      value;

  return retval;
}

int nit_scc_core_calibration_mode_set(nit_scc_core_state_t *state,
                                      uint16_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_scc_core_calibration_mode_offset, value);
...
int nit_scc_core_calibration_mode_get(nit_scc_core_state_t *state,
                                      uint16_t *value) {
  int retval = 0;
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  *value = *((uint32_t *)((uint8_t *)priv->ctrl_stub_shm) +
             nit_scc_core_calibration_mode_offset);
...
int nit_scc_core_stub_eval_calibrate_acquire_min(nit_scc_core_state_t *state) {
  ...
  syslog(LOG_INFO, "nit_scc_core_stub_eval_calibrate_acquire_min\n");
  ...
  auto priv = (scc_core_private_state_t *)state->priv;
  auto frame =
      nit_framebuffer_core_get_memory_map(&nit_framebuffer_core_driver);
...
int nit_scc_core_stub_eval_calibrate_acquire_max(nit_scc_core_state_t *state) {
  ...
  syslog(LOG_INFO, "nit_scc_core_stub_eval_calibrate_acquire_max\n");
...
int nit_scc_core_stub_eval_calibrate_update(nit_scc_core_state_t *state) {
  ...
  syslog(LOG_INFO, "nit_scc_core_stub_eval_calibrate_update\n");
  ...
  if (mean_min == mean_max) {
    syslog(LOG_INFO, "failed to sample two sparse samples, check shutter!\n");
  ...
  for (size_t row = 0; row < state->config.height; row++) {
    for (size_t col = 0; col < state->config.width; col++) {
      ...
      if (x0 == x1) {
        syslog(LOG_INFO, "Dead pixel detected at row: %lu col: %lu\n", row,
               col);
        continue;
      }
...
int nit_scc_core_stub_eval_calibrate(nit_scc_core_state_t *state) {
  ...
  auto &calibration_mode = *((uint32_t *)((uint8_t *)priv->ctrl_shm) +
                             nit_scc_core_calibration_mode_offset);

  syslog(LOG_INFO, "nit_scc_core_stub_eval_calibrate\n");
...
int nit_scc_core_stub_eval_process_eval(nit_scc_core_state_t *state,
                                        volatile int16_t *target,
                                        volatile int16_t *source,
                                        volatile int32_t *scale,
                                        volatile int32_t *offset) {
  for (size_t row = 0; row < state->config.height; row++) {
    for (size_t col = 0; col < state->config.width; col++) {
      size_t index = row * state->config.width + col;
      target[index] = ((double)(source[index] * scale[index] + offset[index])) /
                      ((double)(INT16_MAX >> 1));
    }
  }
...
int nit_scc_core_stub_eval_process(nit_scc_core_state_t *state) {
  ...
  if (state->config.calibration_bypass) {
    // nit_scc_core_stub_eval_process_eval(state, priv->framebuffer_shm,
    // priv->framebuffer_shm, priv->frame_scale_default,
    // priv->frame_offset_default);
  } else {
    nit_scc_core_stub_eval_process_eval(state, priv->framebuffer_shm,
                                        priv->framebuffer_shm, priv->scale_shm,
                                        priv->offset_shm);
  }
...
int nit_scc_core_stub_eval(nit_scc_core_state_t *state) {
  ...
  scc_core_private_state_t *priv = (scc_core_private_state_t *)state->priv;
  auto opmode =
      *((uint32_t *)((uint8_t *)priv->ctrl_shm) + nit_scc_core_opmode_offset);
...
int nit_scc_core_calibration_mode_wait_idle(nit_scc_core_state_t *state) {
  ...
  syslog(LOG_INFO, "calibration core started waiting\n");

  nit_scc_core_calibration_mode_get(state, &result);

  while (result != NIT_SCC_CORE_CALIBRATION_STATUS_IDLE &&
         (std::chrono::high_resolution_clock::now() - start) <
             std::chrono::milliseconds(100)) {

    int retval = *((uint32_t *)((uint8_t *)priv->ctrl_shm) +
                   nit_scc_core_calibration_mode_offset);
  ...
  syslog(LOG_INFO, "calibration code is idle\n");
...
int nit_scc_core_calibrate(nit_scc_core_state_t *state) {
  ...
  syslog(LOG_INFO, "Acquiring minimum....");
  ...
  syslog(LOG_INFO, "Acquiring maximum....");
   ...
   */
  nit_scc_core_calibration_mode_set(
      &nit_scc_core_driver, NIT_SCC_CORE_CALIBRATION_STATUS_ACQUIRING_UPDATING);
