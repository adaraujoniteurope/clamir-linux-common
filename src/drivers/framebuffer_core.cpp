...
public:
  static void gain_table_generate(type *buffer, double dispersion, size_t width,
                                  size_t height) {
    ...
    for (size_t row = 0; row < width; row++) {
      for (size_t col = 0; col < height; col++) {
        auto gain =
            (INT16_MAX >> 1) * normal_distribution(random_number_generator);
        buffer[row * width + col] = gain;
      }
    }
  }

  static void gain_table_apply(type *buffer, type *table, size_t width,
                               size_t height) {

    for (size_t row = 0; row < width; row++) {
      for (size_t col = 0; col < height; col++) {
        auto idx = row * width + col;
        buffer[idx] = (int16_t)(double)buffer[idx] *
                      ((double)table[idx] / (double)(INT16_MAX >> 1));
      }
    }
  }
  ...
  static void generate_circle(type *buffer, size_t x = 32, size_t y = 32,
                              size_t radius = 10, size_t width = 64,
                              size_t height = 64, type low = 500,
                              type high = 2000) {
    for (size_t row = 0; row < height; row++) {
      for (size_t col = 0; col < width; col++) {
        double d = std::sqrt(std::pow((double)col - (double)x, 2) +
                             std::pow((double)row - (double)y, 2));
  ...
  static void generate_uniform(type *buffer, type value = 500,
...
nit_framebuffer_core_state_priv_t *
nit_framebuffer_core_private_state_get(nit_framebuffer_core_state_t *state);
...
int nit_framebuffer_core_open(nit_framebuffer_core_state_t *state,
  ...
  state->priv = (nit_framebuffer_core_state_priv_t *)malloc(
      sizeof(nit_framebuffer_core_state_priv_t));
  ...
  priv->frame = (int16_t *)memory_map_open(
      NULL, NIT_FRAMEBUFFER_CORE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,
      state->fd, NIT_FRAMEBUFFER_CORE_BASE_ADDRESS);
...
int nit_framebuffer_core_config_save_to_file(
    nit_framebuffer_core_state_t *state, const char *path) {
  return 0;
}

int nit_framebuffer_core_config_load_from_file(
    nit_framebuffer_core_state_t *state, const char *path) {
...
volatile int16_t *
nit_framebuffer_core_get_memory_map(nit_framebuffer_core_state_t *state) {
...
int nit_framebuffer_core_run(nit_framebuffer_core_state_t *state,
...
int nit_framebuffer_core_loop(nit_framebuffer_core_state_t *state) {
  ...
  if ((std::chrono::high_resolution_clock::now() - last) >
    ...
    case NIT_FRAMEBUFFER_CORE_OPERATING_MODE_TEST_PATTERN_BALL:
      sensor<int16_t>::generate_circle((int16_t *)frame, x, y, radius, 64, 64,
                                       1000, 2000);
      break;
  ...
  auto m00 = math::algorithm::moments::moments<int16_t>((int16_t *)frame, 64,
                                                        64, 0, 0);
  auto m01 = math::algorithm::moments::moments<int16_t>((int16_t *)frame, 64,
                                                        64, 1, 0);
  auto m10 = math::algorithm::moments::moments<int16_t>((int16_t *)frame, 64,
                                                        64, 0, 1);
  auto m11 = math::algorithm::moments::moments<int16_t>((int16_t *)frame, 64,
                                                        64, 1, 1);
  auto m02 = math::algorithm::moments::moments<int16_t>((int16_t *)frame, 64,
                                                        64, 2, 0);
  auto m20 = math::algorithm::moments::moments<int16_t>((int16_t *)frame, 64,
                                                        64, 0, 2);
...
int nit_framebuffer_core_operating_mode_get(nit_framebuffer_core_state_t *state,
                                            uint16_t *value) {
  int retval = 0;
...
int nit_framebuffer_core_operating_mode_set(nit_framebuffer_core_state_t *state,
                                            uint16_t value) {
  syslog(LOG_INFO, "%s: (0x%04x): %d\n", __func__,
         nit_framebuffer_core_operating_mode_offset, value);
