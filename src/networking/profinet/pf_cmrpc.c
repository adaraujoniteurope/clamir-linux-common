   ...
   pf_ar_t ** pp_ar)
{
   ...
   while (
       ...
       * 
       */
      (ix < NELEMENTS (net->cmrpc_ar)) && ((net->cmrpc_ar[ix].in_use == false)

      /**
       * 
       */
      || ((pf_cmdev_get_state (&net->cmrpc_ar[ix], &cmdev_state) == 0) && (cmdev_state == PF_CMDEV_STATE_POWER_ON))
...
/**
 * @internal
 * Take a DCE RPC connect request and Fcreate a DCE RPC connect response.
 *
   ...
   else if (pf_cmrpc_rm_read_interpret_ind (p_sess, req_pos, &read_request) == 0)
   {
      ...
      else
      {
         ...
         if ((p_ar == NULL) && (opnum == PF_RPC_DEV_OPNUM_READ_IMPLICIT))
         {
            ...
            if (pf_ar_find_by_uuid (net, &read_request.target_ar_uuid, &p_ar) < 0)
            {
               return -1;
            }
