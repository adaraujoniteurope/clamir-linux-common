...
enum class io_value : uint8_t { high, low };

template <typename type> struct is_memory_mapeed_config_type {
...
template <> struct is_memory_mapeed_config_type<config_base> {
...
protected:
  ...
  memory_mapped_device(size_t offset, size_t size,
                       std::string path = "/dev/mem",
                       std::shared_ptr<memory_mapped_device> base = nullptr)
      : m_offset(offset), m_size(size), m_path(path) {}

public:
  virtual ~memory_mapped_device() { close(); }
  ...
  virtual int open() {
    ...
    if ((m_fd = ::open(m_path.c_str(), O_RDWR | O_SYNC)) < 0) {
      throw std::runtime_error(
          std::format("failed to open memory device at {}", m_path));
    }
    ...
    if ((m_priv = (volatile uint8_t *)mmap(NULL, m_size, PROT_READ | PROT_WRITE,
                                           MAP_SHARED, m_fd, m_offset)) ==
        (uint8_t *)-1) {
      throw std::runtime_error(
          std::format("failed to open memory map at {} with size {} at {}",
                      m_offset, m_size, m_path));
    }
  ...
  template <typename type> type &read(size_t offset) {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    if (offset > m_offset) {
      throw std::runtime_error(
          std::format("given offset {} is bigger than device address range "
                      "plus variable size {}",
                      offset, m_offset + sizeof(type)));
    }

    return ((type *)m_priv)[offset];
  }

  template <typename type> type *ref(size_t offset) {
    if (!is_open()) {
      throw std::runtime_error("driver is not open!");
    }
    ...
    return (type *)&(((uint8_t *)m_priv)[offset]);
  }

  template <typename type> void write(size_t offset, type value) {
    std::unique_lock<std::mutex> lk(m_mutex);
