...
namespace nit::embedded::drivers
{
    ...
    public:
        ...
        led_core(led_core_config& config) : memory_mapped_device<led_core>(config.offset, config.size, config.path, nullptr)
        {
            ...
            write<uint16_t>(1, frequency_to_prescaler_value(_config.led_g_frequency));
            write<uint16_t>(5, frequency_to_prescaler_value(_config.led_g_frequency));
            write<uint16_t>(3, frequency_to_prescaler_value(_config.led_b_frequency));

            write<uint16_t>(0, duty_to_prescaler_value(_config.led_r_duty, _config.led_r_frequency));
            write<uint16_t>(4, duty_to_prescaler_value(_config.led_g_duty, _config.led_g_frequency));
            write<uint16_t>(2, duty_to_prescaler_value(_config.led_b_duty, _config.led_b_frequency));
        }
