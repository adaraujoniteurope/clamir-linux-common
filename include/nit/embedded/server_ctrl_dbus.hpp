...
#include <nit/embedded/server_ctrl_config.hpp>
...
class server_ctrl_dbus;

struct command_processor_route
{
	using write_callback_type = std::function<void(std::shared_ptr<server_ctrl_dbus>, command_processor_route &, packet &)>;
	using read_callback_type = std::function<void(std::shared_ptr<server_ctrl_dbus>, command_processor_route &, packet &)>;
	void* pdata;
	read_callback_type read;
	write_callback_type write;
};

class server_ctrl_dbus : std::enable_shared_from_this<server_ctrl_dbus>
{
	server_ctrl_dbus();
...
public:
	...
	virtual ~server_ctrl_dbus()
	{
		syslog(LOG_INFO, __func__);
	}

	int default_handler(const unsigned char *buffer, int);
	static std::shared_ptr<server_ctrl_dbus> get_instance();
	...
	server_ctrl_dbus_config& config_get() { return config; }
...
private:
	...
	void image_writer_legacy();
	void command_processor_legacy();

	utils::signal<std::shared_ptr<server_ctrl_dbus>, uint8_t *, size_t, uint8_t *, size_t> image_read;

	virtual void print_usage();

	static std::shared_ptr<server_ctrl_dbus> instance;

	server_ctrl_dbus_config config_default = {
		.version_major = 6,
		.version_minor = 0,
		.version_patch = 5,
	...
	server_ctrl_dbus_config config;
